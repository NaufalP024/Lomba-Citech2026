...
export function exportCityDataJSON(buildings: BuildingData[]) {
  ...
  downloadAnchor.setAttribute("download", `Laporan_CityOS_Purwakarta_${new Date().toISOString().split('T')[0]}.json`);
...
export function exportCityDataCSV(buildings: BuildingData[]) {
  ...
  const rows = buildings.map(b => [
    ...
    `"${(b.type || '').replace(/"/g, '""')}"`,
    `"${(b.statusLabel || '').replace(/"/g, '""')}"`,
    b.occupancy,
    `"${(b.totalArea || '').replace(/"/g, '""')}"`,
  ...
  // Using semicolon delimiter + UTF-8 BOM so Excel opens clean columns in standard Indonesian/European regional settings
  const csvString = '\uFEFF' + [headers.join(';'), ...rows.map(e => e.join(';'))].join('\r\n');
  const blob = new Blob([csvString], { type: 'text/csv;charset=utf-8;' });
  const url = URL.createObjectURL(blob);
  const downloadAnchor = document.createElement('a');
  downloadAnchor.setAttribute("href", url);
  downloadAnchor.setAttribute("download", `Laporan_CityOS_Purwakarta_${new Date().toISOString().split('T')[0]}.csv`);
  document.body.appendChild(downloadAnchor);
  downloadAnchor.click();
  downloadAnchor.remove();
