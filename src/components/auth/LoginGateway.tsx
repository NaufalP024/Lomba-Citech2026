...
export const LoginGateway: React.FC<LoginGatewayProps> = ({ onLoginSuccess }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 w-full h-screen bg-slate-950 flex flex-col lg:flex-row overflow-hidden select-none">
      
      {/* LEFT COLUMN: Clean White/Slate Form Area (Full screen centered on Mobile HP, Split side on Desktop) */}
      <div className="w-full lg:w-5/12 xl:w-4/12 h-full bg-white dark:bg-slate-900 p-5 sm:p-8 lg:p-10 flex flex-col justify-between z-10 shadow-2xl shrink-0 overflow-hidden">
        <div className="max-w-md mx-auto w-full my-auto py-2">
          ...
          {/* Form Container */}
          <form onSubmit={handleLoginSubmit} className="space-y-3.5 lg:space-y-4">
            ...
            <div>
              ...
              <div className="relative">
                    ...
                    <div className="min-w-0">
                      <div className="flex items-center space-x-2">
                        ...
                        {selectedUser.isSuperAdmin && (
                          <span className="px-2 py-0.5 rounded-full text-[8px] font-black bg-blue-600 text-white uppercase tracking-wider">
                            Superadmin
                          </span>
                        )}
                      </div>
                ...
                {/* Dropdown Menu List */}
                {isDropdownOpen && (
                  <div className="absolute top-full left-0 right-0 mt-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-2xl shadow-2xl z-40 max-h-56 overflow-y-auto p-2 space-y-1">
                        ...
                        <div className="flex items-center space-x-3 min-w-0">
                          ...
                          <div className="min-w-0">
                            <div className="text-xs font-bold truncate flex items-center space-x-1.5">
                              <span>{u.name}</span>
                              {u.isSuperAdmin && (
                                <span className="text-[8px] bg-blue-600 text-white px-1.5 rounded font-extrabold">SA</span>
                              )}
                            </div>
            ...
            <div className="p-3 rounded-2xl bg-blue-50/70 dark:bg-blue-950/40 border border-blue-100 dark:border-blue-900/60 flex items-center space-x-3 text-xs">
              <div className="p-2 rounded-xl bg-blue-600 text-white shrink-0 shadow-sm">
                <Lock className="w-3.5 h-3.5" />
              </div>
              <div>
                <span className="text-[9px] font-extrabold text-blue-600 dark:text-blue-400 uppercase tracking-wider block">
            ...
            <button
              type="submit"
              className="w-full py-3.5 px-6 rounded-full bg-blue-600 hover:bg-blue-700 active:scale-[0.99] text-white font-bold text-xs sm:text-sm shadow-xl shadow-blue-600/30 transition-all flex items-center justify-center space-x-2 mt-3"
            >
          ...
          {/* Footer Subtext */}
          <div className="pt-4 border-t border-slate-100 dark:border-slate-800 text-center sm:text-left mt-6">
      ...
      <div className="hidden lg:flex lg:w-7/12 xl:w-8/12 lg:h-full relative overflow-hidden bg-slate-950 flex-col justify-between p-8 sm:p-12 lg:p-16 shrink-0">
        ...
        <div className="relative z-10 flex items-center justify-between">
          ...
          <span className="hidden sm:inline-flex px-3 py-1.5 rounded-full bg-blue-600 text-white text-[11px] font-extrabold uppercase tracking-wider backdrop-blur-md">
            Digital Twin 3D
          </span>
        </div>
        ...
        <div className="relative z-10 max-w-2xl mt-auto">
          ...
          <div className="grid grid-cols-3 gap-3 pt-6 border-t border-white/20">
            ...
            <div className="bg-slate-900/70 border border-white/10 rounded-2xl p-3.5 backdrop-blur-md">
              <span className="text-lg sm:text-2xl font-extrabold text-blue-400 block">11 Personel</span>
              <span className="text-[10px] sm:text-xs text-blue-200 font-medium">Tim Pengelola</span>
            </div>
            <div className="bg-slate-900/70 border border-white/10 rounded-2xl p-3.5 backdrop-blur-md">
              <span className="text-lg sm:text-2xl font-extrabold text-blue-400 block">Real-time</span>
              <span className="text-[10px] sm:text-xs text-blue-300 font-medium">Telemetri Kota</span>
            </div>
          </div>
        </div>
